...
- Schritte: apt-Abhängigkeiten → Checkout mit Submodulen → `cmake`/`ninja` (mit `-Werror`) → `ctest` → clang-tidy-Gate → `sanitize-address` → `sanitize-memory`. Ist der Laptop aus, bleibt der Job in der Queue, bis der Runner wieder pollt.
- Die beiden Sanitizer-Schritte bauen den Baum je einmal neu — ein CI-Lauf kompiliert das Projekt also dreimal. `sanitize-memory` läuft mit `continue-on-error: true`: er bricht mit dem bekannten Fehlalarm aus dem nicht instrumentierten libstdc++/gtest ab, sein Bericht steht aber im Joblog. Nur `sanitize-address` bricht den Job.
