...
public:
  ...
  void add_child(const std::shared_ptr<SceneNode> &child) { children.push_back(child); }
  ...
  [[nodiscard]] const std::vector<std::shared_ptr<SceneNode>> &get_children() const
  {
    return children;
  }
  ...
  void set_shape(std::shared_ptr<Shape> shape_ptr) { shape = std::move(shape_ptr); }
