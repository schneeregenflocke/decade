...
public:
  ...
  void draw() const override
  {
    ...
    if (colors.size() == 2) {
      shader()->SetUniform("outline_color", colors[0]);
      shader()->SetUniform("fill_color", colors[1]);
    }
...
private:
                     ...
                     const glm::vec3 &point2, const glm::vec3 &point3)
  {
    ...
    vertices[offset + 2] = point2;
    vertices[offset + 3] = point3;
    vertices[offset + 4] = point2;
    vertices[offset + (kVerticesPerQuad - 1)] = point1;
  }
