...
public:
  ...
  ShaderInfo(InfoType info_type_value, std::string name_value, InfoParams params)
      : info_type(info_type_value), name(std::move(name_value)), location(params.location),
...
private:
  ...
  void GatherAttributesInfo()
  {
    ...
    for (int index = 0; index < num_attribs; ++index) {
      std::vector<char> attrib_name(static_cast<size_t>(max_attrib_name_length), '\0');
      GLsizei written = 0;
      GLint size = 0;
      GLenum type = 0;
  ...
  void GatherUniformsInfo()
  {
    ...
    for (int index = 0; index < num_uniforms; ++index) {
      std::vector<char> uniform_name(static_cast<size_t>(max_uniforms_name_length), '\0');
      GLsizei written = 0;
      GLint size = 0;
      GLenum type = 0;
