...
public:
	...
	ImageComposer(size_t width, size_t height, const rectf& ortho_dimension, std::shared_ptr<GraphicsEngine> graphics_engine, int msaa_samples) :
		...
		number_width_parts(0),
		number_height_parts(0),
		pixel_size(0),
		standard_size(0),
		width_remainder(0),
		height_remainder(0),
	...
	ImagePart& ImagePartRef(size_t x, size_t y)
	{
		return image_parts[x + y * number_width_parts];
	}

	std::vector<unsigned char> CopyImage() const
	{
		return image;
	}

	size_t NumberImageParts() const
	{
		return image_parts.size();
	}

	size_t NumberWidthParts() const
	{
		return number_width_parts;
	}

	size_t NumberHeightParts() const
	{
		return number_height_parts;
	}
...
private:
	...
	void CalculateOrthoRemainder()
	{
		...
		float height_remainder_ratio = static_cast<float>(height_remainder) / static_cast<float>(height);
	...
	void ComposeImageParts()
	{
		...
		for (size_t y_index = 0; y_index < NumberHeightParts(); ++y_index)
		{
			for (size_t x_index = 0; x_index < NumberWidthParts(); ++x_index)
			{
				...
				auto current_image_position = current_height_position * width * pixel_size + current_width_position * pixel_size;
				...
				for (size_t index = 0; index < current_image_part.viewport[1]; ++index)
				{
					...
					std::copy(current_image_part.image.cbegin() + current_x_begin, current_image_part.image.cbegin() + current_x_end,
	...
	void VerticalFlip()
	{
		...
		for (size_t index = 0; index < height; ++index)
		{
			...
			std::copy(image_copy.cbegin() + current_row_begin, image_copy.cbegin() + current_row_end, image.end() - current_row_end);
		}
	}
...
private:
	...
	size_t MaxPngHeight()
	{
		return PNG_UINT_32_MAX / (sizeof(png_bytep));
	}
	...
	size_t MaxPngSize() const
	{
		return PNG_SIZE_MAX;
	}

	void SavePicture(const char* file_name, std::vector<unsigned char>& image, size_t width, size_t height)
	{
