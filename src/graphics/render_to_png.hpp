...
private:
  ...
  void CalculateNumberImageParts()
  {
    ...
    number_width_parts = number_width_parts_without_remainder + (width_remainder > 0 ? 1 : 0);
    number_height_parts = number_height_parts_without_remainder + (height_remainder > 0 ? 1 : 0);
  ...
  void ComposeImageParts()
  {
    ...
    for (size_t y_index = 0; y_index < NumberHeightParts(); ++y_index) {
      for (size_t x_index = 0; x_index < NumberWidthParts(); ++x_index) {
        ...
        const auto viewport_height = static_cast<size_t>(current_image_part.viewport[1]);
        for (size_t index = 0; index < viewport_height; ++index) {
          const size_t x_size = static_cast<size_t>(current_image_part.viewport[0]) * pixel_size;
          const size_t current_x_begin = index * x_size;
          const size_t current_x_end = current_x_begin + x_size;

          const auto image_begin_offset =
              static_cast<std::ptrdiff_t>(current_image_position + (index * width * pixel_size));
          const auto current_x_begin_offset = static_cast<std::ptrdiff_t>(current_x_begin);
          const auto current_x_end_offset = static_cast<std::ptrdiff_t>(current_x_end);
  ...
  void VerticalFlip()
  {
    ...
    for (size_t index = 0; index < height; ++index) {
      ...
      const auto row_begin_offset = static_cast<std::ptrdiff_t>(current_row_begin);
      const auto row_end_offset = static_cast<std::ptrdiff_t>(current_row_end);
      std::copy(image_copy.cbegin() + row_begin_offset, image_copy.cbegin() + row_end_offset,
...
public:
  ...
  void RenderPicture()
  {
    ...
    const bool image_too_large =
        (image_height > max_height) || (image_height > max_height_from_width) ||
        (image_width > max_width_from_height);
    if (image_too_large) {
      return;
    }

    const ImageComposer image(ImageSize{image_width, image_height}, ortho_dimensions,
                              graphics_engine, msaa_samples);
    auto image_data = image.CopyImage();
    SavePicture(file_path.c_str(), image_data, PngSize{image_width, image_height});
  }
...
private:
  ...
  static void SavePicture(const char *file_name, std::vector<unsigned char> &image,
                          PngSize size)
  {
    ...
    png_set_IHDR(png_ptr, info_ptr, png_width, png_height, 8, PNG_COLOR_TYPE_RGB_ALPHA,
                 PNG_INTERLACE_NONE,
    ...
    for (size_t index = 0; index < size.height; ++index) {
      png_bytep current_row_ptr =
          image.data() + index * size.width * static_cast<size_t>(kBytesPerPixel);
