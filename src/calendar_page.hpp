...
public:
  ...
  {
    this->page_size = rectf::from_dimension(rectf::Dimension{page_setup_config.size[0], page_setup_config.size[1]});
  ...
  void Update()
  {
    ...
    const rectf calendar_frame_margin = rectf(kZero, kDefaultMargin, kZero, kZero);
    ...
    if (calendar_config.IsAutoCalendarSpan() && !data_store.is_empty()) {
      calendar_config.SetSpan(
          CalendarSpan::YearSpan{data_store.GetFirstYear(), data_store.GetLastYear()});
    }

    const size_t additional_rows = 2;
    const size_t number_rows =
        additional_rows + static_cast<size_t>(calendar_config.GetSpanLengthYears());

    cell_width = calendar_frame.width() / kCalendarColumns;
    row_height = calendar_frame.height() / static_cast<float>(number_rows);

    const rectf cells_frame_margin(cell_width, kZero, row_height * kRowHeaderScale, kZero);
    ...
    proportion_frame_layout.SetupSubFrames(calendar_config.GetSpacingProportions());

    day_width = cells_frame.width() / kDaysPerYear;

    x_labels_frame =
        calendar_frame.reduce(rectf(cell_width, kZero, row_height, cells_frame.height()));
    y_labels_frame =
        calendar_frame.reduce(rectf(kZero, cells_frame.width(), row_height * kRowHeaderScale,
                                     kZero));
    legend_frame =
        calendar_frame.reduce(rectf(cell_width, kZero, kZero, cells_frame.height() + row_height));
  ...
  void SetupTitleShape()
  {
    ...
    title_shape->set_shape_centered(title_config.TitleText(), title_frame.getCenter(),
  ...
  void SetupCalendarLabelsShape()
  {
    ...
    labels_font_size = font->AdjustTextSize(rectf::from_dimension(rectf::Dimension{cell_width, row_height}),
                                            "00000",
    ...
    auto column_node = scene_graph->search_node("column label area").value_or(nullptr);
    if (!column_node) {
      return;
    }
    ...
    for (int index = 0; index < span_years; ++index) {
      const std::string current_year_text = std::to_string(calendar_config.GetYear(index));
      ...
      year_text_shape->set_shape_centered(current_year_text,
                                          y_labels_frames.at(frame_index).getCenter(),
                                          labels_font_size);
    }

    auto row_node = scene_graph->search_node("row label area").value_or(nullptr);
    if (!row_node) {
      return;
    }
    auto row_shape = std::dynamic_pointer_cast<RectanglesShape>(row_node->get_shape());
    if (!row_shape) {
      return;
    }
  ...
  void SetupMonthsShapes()
  {
    ...
    for (int index = 0; index < span_years; ++index) {
      const int current_year = calendar_config.GetYear(index);
      const boost::gregorian::date first_day_of_year =
          boost::gregorian::date(current_year, 1, 1);
      ...
      for (size_t subindex = 0; subindex < number_months; ++subindex) {
        ...
        const auto end_offset =
            static_cast<float>(boost::gregorian::date_period(
                                   first_day_of_year,
                                   first_day_of_year + boost::gregorian::months(month_index + 1))
                                   .length()
                                   .days()) *
            day_width;
        ...
        const auto store_index = (static_cast<size_t>(index) * number_months) + subindex;
    ...
    for (int index = 0; index < span_years; ++index) {
      ...
      for (std::int64_t subindex = 0; subindex < number_days; ++subindex) {
        ...
        const boost::gregorian::date current_date =
            ...
            boost::gregorian::date_duration(static_cast<DurationRep>(days_index));
  ...
  void SetupBarsShape()
  {
    ...
    for (size_t index = 0; index < number_bars; ++index) {
      ...
      if (calendar_config.IsInSpan(bar.GetYear())) {
        ...
        const auto current_sub_cell = proportion_frame_layout.GetSubFrame(row, 1);

        rectf bar_cell;
        const auto bar_left = current_sub_cell.l() + (bar.GetFirstDay() * day_width);
        const auto bar_right = current_sub_cell.l() + (bar.GetLastDay() * day_width);
  ...
  void SetupYearsTotals()
  {
    ...
    for (int index = 0; index < span_years; ++index) {
      ...
      if (calendar_config.IsInSpan(current_year)) {
        const int row = current_year - calendar_config.GetSpanLimitsYears().at(0);
        ...
        const auto year_total_width =
            static_cast<float>(data_store.GetAnnualTotal(span_index)) * day_width;
        ...
        const float percent = static_cast<float>(data_store.GetAnnualTotal(span_index)) /
                              static_cast<float>(number_days);

        std::ostringstream year_total_stream;
        year_total_stream << std::fixed << std::setprecision(1) << percent * kPercentScale
                          << " %";
  ...
  void SetupLegend()
  {
    ...
    const size_t number_entrie_frames = (date_group_store.GetDateGroups().size() + 1) * 2;
    std::vector<rectf> legend_entries_frames(number_entrie_frames);
    const auto entries_width = legend_frame.width() / static_cast<float>(number_entrie_frames);
    ...
    for (size_t index = 0; index < date_group_store.GetDateGroups().size(); ++index) {
      ...
      shape_text->set_shape_centered(date_group_store.GetDateGroups().at(index).GetName(),
                                     legend_entries_frames.at(label_index).getCenter(),
                                     legend_font_size);

      if (span_years > 0) {
        const auto current_height = proportion_frame_layout.GetSubFrame(0, 1).height();
...

    {
      ...
      shape_text->set_shape_centered(
          "Annual sum", legend_entries_frames.at(legend_entries_frames.size() - 2).getCenter(),
          legend_font_size);

      if (span_years > 0) {
        const auto current_height = proportion_frame_layout.GetSubFrame(0, 0).height();
        auto current_cell = legend_entries_frames.at(legend_entries_frames.size() - 1);
        const auto current_vertical_center = current_cell.getCenter()[1];
        current_cell.setB(current_vertical_center - (current_height * kHalf));
        current_cell.setT(current_vertical_center + (current_height * kHalf));
        bars_cells.emplace_back(current_cell);
