...
public:
  explicit LicenseInformationDialog(wxWindow *parent)
      : wxDialog(parent, wxID_ANY, L"Open Source Licenses Information", wxDefaultPosition,
                 wxSize(kDefaultWidth, kDefaultHeight) /*wxDefaultSize*/,
                 wxCAPTION | wxRESIZE_BORDER | wxMAXIMIZE_BOX)
  {

    license_select_list_box =
        std::make_unique<wxListBox>(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0, nullptr,
                                    wxLB_SINGLE | wxLB_NEEDED_SB)
            .release();

    text_view_ctrl = std::make_unique<wxTextCtrl>(this, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                                  wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY)
                         .release();

    const wxSizerFlags flags0 = wxSizerFlags().Proportion(1).Expand();
    const wxSizerFlags flags1 = wxSizerFlags().Proportion(1).Expand().Border(wxALL, kBorderSize);
    const wxSizerFlags flags2 = wxSizerFlags().Proportion(0).Expand().Border(wxALL, kBorderSize);
    const wxSizerFlags flags3 = wxSizerFlags().Proportion(0).Border(wxALL, kBorderSize).Right();

    auto *vertical_sizer = std::make_unique<wxBoxSizer>(wxVERTICAL).release();
    SetSizer(vertical_sizer);

    auto *horizontal_sizer = std::make_unique<wxBoxSizer>(wxHORIZONTAL).release();
...
private:
  ...
  void CollectLicenses()
  {
    ...
    collected_licenses.emplace_back("embed-resource",
                                    LOAD_RESOURCE(embed_resource_LICENSE_LICENSE).toString());
    collected_licenses.emplace_back("csv2", LOAD_RESOURCE(csv2_copyright_LICENSE).toString());
    collected_licenses.emplace_back("csv2mio", LOAD_RESOURCE(csv2mio_LICENSE_LICENSE).toString());
    collected_licenses.emplace_back("sigslot", LOAD_RESOURCE(sigslot_LICENSE_LICENSE).toString());
  ...
  void SlotSelectLicense(wxCommandEvent &event) { SelectLicense(event.GetString().ToStdString()); }
  ...
  {
    auto iter = std::find_if(collected_licenses.cbegin(), collected_licenses.cend(),
