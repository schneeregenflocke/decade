...
Two compilers, two trees, and the split is deliberate: GCC builds what ships (`build/`), clang carries the tools that parse with its own frontend (`build-clang/` — `clang-tidy` and `sanitize-memory`, which CMake defines under clang alone). CI runs one job per compiler, and both build and test, because each sees warnings the other does not. Whoever adds a gate decides which tree it belongs to.
