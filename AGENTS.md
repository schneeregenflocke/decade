...
That is why **no class carries `Q_OBJECT`, and `AUTOMOC` stays off**: [moc](https://doc.qt.io/qt-6/moc.html) would emit a second translation unit. Nothing here needs it — the application's own signals run over sigslot, subscribing to a Qt widget's signal works through `QObject::connect` with a functor, and event handlers are plain virtual overrides. Declaring signals and slots needs `Q_OBJECT`, and no class here declares any. A new class that seems to want one wants sigslot instead.

### OpenGL under Qt

Two traps, both silent, both settled in one place each.
