...
Diese Datei enthält Anweisungen für Claude Code (claude.ai/code) und andere Coding-Agents, die in diesem Repository mit dem Code arbeiten. Sie hält die stabilen Leitplanken — Bauen, Architektur, Konventionen, Prinzipien. Offene Punkte stehen als Issues.
...
Diese Datei hält die stabilen Leitplanken. Offene Aufgaben, Fragen und Review-Befunde
stehen als Issues, nie als Zeile hier:
...
Jedes neue Issue hängt ins Project
[Offene Aufgaben](https://github.com/users/schneeregenflocke/projects/4).
Erledigtes liegt geschlossen mit Label `historie`:
