...
- **Presentation** — the main window (`MainWindow`), Qt panels, the GL canvas (`QOpenGLWidget`), menus, dialogues, file commands. Every panel owns its widgets and declares a `Q_OBJECT` signal for what the user edited — carrying the same value its store's topic publishes back. The window owns widgets alone: no stores, no bus, no runtime options.
...
- **Domain** — value objects, stores, transformation logic, state topics — widget-free and Boost-free (no `friend boost::serialization::access`, no `serialize` members).
...
4. The domain may know **QtCore**, and nothing else of the outer layers — no QtWidgets, no QtGui, no GL, no Boost. The test asks what a library binds to, not its name: a dependency drops out when it ties the domain to an outer layer — a widget toolkit, a graphics context, a file format. That admits glm, which carries the domain colours as `glm::vec4`: header-only maths, no GL header anywhere underneath it, nothing to link ([#58](https://github.com/schneeregenflocke/decade/issues/58)). QtCore came in with the state topics, and it passes the same test: a Qt signal has to live in a `QObject`, `QObject` is QtCore alone, and the domain therefore still builds and runs without a window. The rule read "no Qt" until [#86](https://github.com/schneeregenflocke/decade/issues/86) weighed the alternative — a QObject bus in the application plus an adapter per topic — and found it more indirection than one mechanism was worth.
...
9. Exactly one bridge connects application and rendering infrastructure: the rendering adapter with its scene composer. Presentation depends on no GL type, with one exception: the GL canvas (`GLCanvas` with its `MouseInteraction`). As the `QOpenGLWidget` it owns the context, and whatever lives in that context lives with it — the renderer (`GraphicsEngine`), camera and projection, the PNG export — because a GL object has to die while the canvas can still make its context current. Every other part of presentation sees the scene graph as a GL-free read model (a snapshot) alone. The way back runs over a port the application declares and presentation implements (`RenderSurface`: the adapter asks for a repaint without knowing the canvas).
...
- **Stores** combine a value object with an injected state topic and a re-entry guard. They have identity → explicitly not copyable. A store owns no signal of its own: it gets its topic by constructor reference (the port, `domain/state_topics.hpp`) and publishes straight onto it with `Publish`. Who owns the topic is the outer layer's decision — the `EventBus` in the application, a local topic in a test. Each topic is a small `QObject` carrying one value on one `Published` signal, so consumers work with copyable value objects; the store offers `Receive`, `Send` and `Get`, no query delegation of its own. One class per value rather than one template, because moc processes no class template — the two topics carrying a `PickId` sit in the application instead (`application/interaction_topics.hpp`), since that type is infrastructure.
...
### Event flow (the event bus through Qt signals)

Communication across components runs over an in-process **event bus** with one typed topic per domain event. Stores publish straight onto it (see [Domain pattern](#domain-pattern-value-objects-and-stores)); the wiring of the consumers sits centrally in `app_binder::Bind`. You add a new piece of state as a Boost-free value object plus a `*Store` in `domain/`, a topic class in `domain/state_topics.hpp` (a `QObject` with one `Publish` and one `Published`), a panel in `presentation/`, the topic as a member of the `EventBus` and the consumers in the binder; persisted state adds `save`/`load` in `infrastructure/persistence/value_serialization.hpp` and one line in `project_io`.

**Two directions, two rules.** A panel edit is a **command** and goes straight to the owning store — its `Receive*` is the only place canonical state comes into being. The new state is a **fact** and goes over the bus. Were panels to put their edits onto the same topic they subscribe to, there would be feedback loops; the panel signals are named for the edit (`…Edited`, `…Chosen`) and the topics for the fact (`Published`), so the two directions do not read alike. Only where a producer sits in presentation and therefore gets no topic injected (font choice, tree selection) does the binder connect the panel signal to the topic's own `Publish`. The scene tree is the counter-check: it mirrors the render graph and only displays — every row of its detail grid is read-only, because an edit field there would be a second write path onto state the rebuild recreates anyway. The shape configurations get edited where they live, in the Shapes panel.
...
**One topic, several consumers: the binder's call order is the delivery order.** Qt activates the slots of one signal "in the same order as the order the connection was made" ([QObject::connect](https://doc.qt.io/qt-6/qobject.html#connect)), and one place depends on it — the shape store answers a new date group by synthesising its configurations, and that has to land before the rendering adapter rebuilds the scene from them. Reordering the lines in `app_binder` therefore changes behaviour, quietly. Whoever adds a consumer to a topic that already has one asks first whether it may run last.

The wiring itself is a lifetime, not a pair of calls: `AppWiring` connects on construction and releases on destruction. It needs no list of disconnects for that — every connection the binder makes carries a plain `QObject` member of `AppWiring` as its **context object**, and Qt drops them all when that object dies. What the context cannot reach releases by hand in `ReleaseCallbacks`: the canvas, controller and editor callbacks are `std::function`s rather than signals, because a pick source answers with a hit and a Qt signal carries no return value. In the composition root the wiring stands as the last member and thereby dies before stores and bus.
