...
Reconfigure (only when `CMakeLists.txt` or a dependency changes) — the first line for GCC, the second for clang; the second overwrites the first in the same directory:
...
`cmake --install` places the binary and the licence texts the dialogue also carries:

```bash
cmake -S . -B build -G Ninja -DCMAKE_INSTALL_PREFIX=/some/where
cmake --build build
cmake --install build            # bin/decade, share/decade/licenses/
```
...
- `--dump-png=<path>` — the **drawing** alone, through an off-screen FBO: the page, grown by whatever of a calendar sized in millimetres runs past it. Resolution: the export DPI, a white background, no app chrome. Needs: OpenGL alone. The 16 multisamples it asks for get capped at what the driver offers — 16 on the NVIDIA GPU, 8 under llvmpipe — so a headless export is a little coarser than one off the graphics card, but never black ([#49](https://github.com/schneeregenflocke/decade/issues/49)). `--debug-log` names the count actually used.
- `--dump-window-png=<path>` — the **whole window**: tabs plus panels (`QWidget::grab`) with the canvas content composed on top. Resolution: screen resolution. `grab()` renders the widgets rather than reading the screen, and `QOpenGLWidget` draws into a framebuffer object that can be read back — so this needs no X11 any more and works under Wayland too.

Take `--dump-png` for a clean high-DPI export of the drawing itself, `--dump-window-png` for the real GUI including chrome. `--dump-window-png` gets queued on the event loop, so the first paint has already happened.
...
```bash
clang-tidy -p build --fix --fix-notes \
...
clangd is the third reader of the sources, and unlike the two gates it reports into an editor nobody else is sitting in front of. `--check` parses one file the way the language server would and prints to the terminal:

```bash
clangd --check=src/domain/date.cpp 2>&1 | grep -E "Line [0-9]+:|All checks"
```

The filter matters: the output is a whole session log, the diagnostics are the lines carrying a source position, and the `tweak: … ==> FAIL` lines are `--check` probing code actions, not findings. Over a set of files, one at a time — there is no project-wide mode:

```bash
for f in src/domain/*.cpp; do clangd --check="$f" 2>&1 | grep -E "Line [0-9]+:" | sed "s|^|$f |"; done
```

`--check` reports **compiler** diagnostics alone. clangd does run clang-tidy and does read `.clang-tidy` — a `.clangd` `Diagnostics.ClangTidy` block would merge on top with precedence — but those findings reach the LSP client and never `--check`; measured against clangd 22.1.8, where a deliberate `readability-braces-around-statements` violation shows up over the protocol and stays invisible on the command line. That is no gap in coverage: the clang-tidy gate reads the same `.clang-tidy` over the whole of `src/` and grades every finding as an error.

Whichever compiler `build/` holds is the one clangd sees. Under GCC it meets `-mno-direct-extern-access`, and `.clangd` cuts that with `CompileFlags.Remove`; the file reaches clangd alone, and under clang it does nothing. Unknown GCC *warning* options need no counterpart, because clangd drops those diagnostics silently.
...
cmake --build build --target sanitize-memory    # diagnosis, needs a clang build/
```
...
CI runs on our own [Forgejo instance](https://forgejo.blem.ch/), not on GitHub Actions. The old workflow file `.github/workflows/cmake.yml` runs on demand alone (`on: workflow_dispatch`, so `gh workflow run cmake.yml`) and takes part in no push. The chain, analogous to `blem-website`:
...
curl -s -H "Authorization: token $FJ" \
  "https://forgejo.blem.ch/api/v1/repos/github-mirror/decade/actions/tasks?page=1&limit=10" \
